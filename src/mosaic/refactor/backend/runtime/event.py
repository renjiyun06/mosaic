"""Event system for Mosaic runtime layer"""
import json
...
class MosaicEvent(BaseModel, ABC):
    """Base class for all Mosaic events"""
    ...
    def to_llm_message(self) -> str:
        """Convert event to LLM-readable message format"""
        ...

    @abstractmethod
    def to_batch_message(self) -> str:
        """Convert event to batch message format"""
        ...
...
class NodeMessage(MosaicEvent):
    ...
    def to_batch_message(self) -> str:
        return f"<node_message>{self.payload.get('message')}</node_message>"
...
# Event registry: maps event type to event class
EVENTS = {
