...
from mosaic.core.enums import MeshStatus, NodeType
...
def get_subscriptions_by_source_and_event_pattern(mesh_id: MeshID, source_id: NodeID, event_pattern: str) -> List[Subscription]:
    with _get_conn() as conn:
        rows = conn.execute("SELECT * FROM subscriptions WHERE mesh_id = ? AND source_id = ? AND event_pattern = ?", (mesh_id, source_id, event_pattern)).fetchall()
        return [
            Subscription(mesh_id=row["mesh_id"], source_id=row["source_id"], target_id=row["target_id"], event_pattern=row["event_pattern"], is_blocking=row["is_blocking"], session_routing_strategy=SessionRoutingStrategy(row["session_routing_strategy"]), session_routing_strategy_config=json.loads(row["session_routing_strategy_config"]))
            for row in rows
        ]
