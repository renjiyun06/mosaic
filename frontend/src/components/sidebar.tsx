...
export function Sidebar({ mosaicId }: SidebarProps) {
  ...
  const { setMosaicStatus } = useMosaicStore()
  ...
  useEffect(() => {
    ...
    const fetchMosaic = async () => {
      try {
        setLoading(true)
        const data = await apiClient.getMosaic(parseInt(mosaicId), token)
        setMosaic(data)
      } catch (error) {
  ...
  const handleStart = async () => {
    ...
    try {
      setOperating(true)
      const updated = await apiClient.startMosaic(mosaic.id, token)
      setMosaic(updated)
      // Update global store to trigger node list refresh
      setMosaicStatus(mosaicId, "running")
    } catch (error) {
  ...
  const confirmStop = async () => {
    ...
    try {
      setOperating(true)
      const updated = await apiClient.stopMosaic(mosaic.id, token)
      setMosaic(updated)
      setStopConfirmOpen(false)
      // Update global store to trigger node list refresh
      setMosaicStatus(mosaicId, "stopped")
    } catch (error) {
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              <div className="flex items-start justify-between">
                ...
                <Badge
                  variant={mosaic.status === "running" ? "default" : "secondary"}
                  className="ml-2"
                >
                  <Circle
                    className={cn(
