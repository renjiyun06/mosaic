...
export default function Home() {
  ...
  const [mosaics, setMosaics] = useState<MosaicResponse[]>([])
  ...
  useEffect(() => {
    ...
    const fetchMosaics = async () => {
      try {
        setLoading(true)
        const data = await apiClient.listMosaics(token)
        setMosaics(data)
      } catch (error) {
  ...
  const handleCreateMosaic = async () => {
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : "Failed to create mosaic")
    } finally {
      setCreating(false)
    }
  }
  ...
  const handleStartMosaic = async (mosaicId: number, e: React.MouseEvent) => {
    ...
    try {
      await apiClient.startMosaic(mosaicId, token)
      // Refresh mosaic list
      const data = await apiClient.listMosaics(token)
      setMosaics(data)
    } catch (error) {
      console.error("Failed to start mosaic:", error)
      alert(error instanceof Error ? error.message : "Failed to start mosaic")
    }
  }
  ...
  const confirmStopMosaic = async () => {
    if (!token || mosaicToStop === null) return

    try {
      await apiClient.stopMosaic(mosaicToStop, token)
      // Refresh mosaic list
      const data = await apiClient.listMosaics(token)
      setMosaics(data)
      ...
      alert(error instanceof Error ? error.message : "Failed to stop mosaic")
    }
  }

  const handleEditMosaic = (mosaic: MosaicResponse, e: React.MouseEvent) => {
    e.preventDefault() // 阻止 Link 跳转
  ...
  const handleUpdateMosaic = async () => {
    if (!token || editingMosaicId === null) return
    if (!formData.name.trim()) return

    try {
      setUpdating(true)
      await apiClient.updateMosaic(
        editingMosaicId,
        {
          name: formData.name,
          description: formData.description || undefined,
      ...
      alert(error instanceof Error ? error.message : "Failed to update mosaic")
    } finally {
      setUpdating(false)
    }
  }
  ...
  const confirmDeleteMosaic = async () => {
    if (!token || mosaicToDelete === null) return

    try {
      setDeleting(true)
      await apiClient.deleteMosaic(mosaicToDelete, token)
      // Refresh mosaic list
      const data = await apiClient.listMosaics(token)
      setMosaics(data)
      ...
      alert(error instanceof Error ? error.message : "Failed to delete mosaic")
    } finally {
      setDeleting(false)
    }
  }
